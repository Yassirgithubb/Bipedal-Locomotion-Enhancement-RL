...
from legged_gym.common.assets.asset_cfg import AssetCfg, CuboidAssetCfg, SphereAssetCfg, FileAssetCfg
...
class Asset:
    ...
    def spawn(self, env_id: int, pos: Sequence[float] = None, quat: Sequence[float] = None):
        ...
        self._sim_actor_ids[env_id] = self.gym.get_actor_index(env_handle, actor_id, gymapi.IndexDomain.DOMAIN_SIM)
...
class FileAsset(Asset):
    ...
    def _retrieve_asset_instance(self, asset_options: gymapi.AssetOptions) -> gymapi.Asset:
        # resolve paths
        asset_path = self.cfg.file.format(LEGGED_GYM_ROOT_DIR=LEGGED_GYM_ROOT_DIR)
