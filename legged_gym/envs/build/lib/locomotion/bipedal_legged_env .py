...
from legged_gym.envs.locomotion.bipedal_legged_env import LeggedEnvCfg
    ...
    def _reset_robot(self, env_ids):
        ...
        if self.cfg.randomization.max_init_roll_pitch > 0.0 or self.cfg.randomization.max_init_yaw > 0.0:
            ...
            pitch = torch.empty(len(env_ids), device=self.device).uniform_(
                -np.pi/2-0.2-self.cfg.randomization.max_init_roll_pitch, -np.pi/2-self.cfg.randomization.max_init_roll_pitch-0.2
            )
